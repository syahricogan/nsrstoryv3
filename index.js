...
* Author nya Radya, Farid, M.hadi.firmansya, and Syahri
* Tambahin ya zhayank
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code above jangan lupa subrek NSR STORY'))
	})

	fs.existsSync('./Syahri.json') && client.loadAuthInfo('./Syahri.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Syahri.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285893123964@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6285893123964@s.whatsapp.net'
                        /******ApiKey Input******/
